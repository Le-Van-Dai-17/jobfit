...
export function AssessmentStartForm({
  ...
  return (
    <form action={formAction} className={hideSelections ? "" : "grid gap-4 rounded-lg border border-border-light bg-surface-white p-4 md:grid-cols-[1fr_1fr_auto]"}>
      {applicationId && <input type="hidden" name="applicationId" value={applicationId} />}

      {hideSelections ? (
        <>
          {selectedResumeVersionId && <input type="hidden" name="resumeVersionId" value={selectedResumeVersionId} />}
          {selectedJobId && <input type="hidden" name="jobId" value={selectedJobId} />}
        </>
      ) : (
        <>
      ...
      <div className={hideSelections ? "" : "flex items-end"}>
        <Button type="submit" isLoading={pending} disabled={disabled} className={hideSelections ? "" : "w-full md:w-auto"}>
      ...
      {state.status === "error" && (
        <p className={`${hideSelections ? "mt-2" : "md:col-span-3"} rounded-md bg-red-50 p-3 text-sm text-red-700`} aria-live="polite">
          {state.message}
        </p>
      )}
    </form>
