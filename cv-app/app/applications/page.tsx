...
export default async function ApplicationsPage({ searchParams }: { searchParams?: Promise<{ applied?: string | string[]; q?: string | string[]; status?: string | string[] }> }) {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl bg-surface-white p-5 shadow-card md:p-7"><p className="text-sm font-semibold text-primary">Ứng tuyển</p><h1 className="mt-2 text-2xl font-bold text-foreground md:text-3xl">Đơn ứng tuyển của tôi</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-text-muted">Theo dõi đúng trạng thái đã lưu, CV snapshot đã nộp và bài đánh giá theo từng JD.</p>
