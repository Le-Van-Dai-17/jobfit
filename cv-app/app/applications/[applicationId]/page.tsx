...
export default async function ApplicationDetailPage({ params, searchParams }: { params: Promise<{ applicationId: string }>; searchParams?: Promise<{ applied?: string }> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl bg-surface-white p-5 shadow-card"><div className="flex items-center gap-2"><ClipboardCheck className="h-5 w-5 text-primary" /><h2 className="text-lg font-bold text-foreground">Bài đánh giá theo đơn</h2></div><p className="mt-2 text-sm leading-6 text-text-muted">Bài đánh giá dùng JD và CV snapshot của đơn này, với kết quả tư vấn dựa trên rubric và bằng chứng.</p>{latestSession ? <Link href={`/assessments/${latestSession.id}`} className="mt-4 inline-flex rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary-hover">Mở phiên đánh giá</Link> : application.resumeVersionId ? <Link href={`/assessments?applicationId=${application.id}&jobId=${application.jobId}&resumeVersionId=${application.resumeVersionId}`} className="mt-4 inline-flex rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary-hover">Tạo bài đánh giá</Link> : null}</section>
