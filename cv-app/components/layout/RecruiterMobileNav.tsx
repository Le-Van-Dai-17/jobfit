...
const items = [
  ...
  ["Ứng viên", "/recruiter/candidates", Users], ["Công ty", "/recruiter/company", Building2],
] as const;
export function RecruiterMobileNav() {
  const pathname = usePathname();
  return <nav aria-label="Điều hướng nhà tuyển dụng" className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-4 border-t border-outline-variant/60 bg-white/95 px-1 py-2 backdrop-blur md:hidden">{items.map(([name,href,Icon]) => {
